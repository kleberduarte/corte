...
export default function PhoneScreen({ items, slotTime, immediate = false, onConfirm }: Props) {
  ...
  return (
    ...
    <div className="scroll" style={{ flex: 1, padding: '16px 28px 28px' }}>
      ...
      <div style={{ fontSize: 'calc(14px * var(--font-scale))', color: 'var(--t3)', lineHeight: 1.5, marginBottom: 22 }}>
      ...
      <div style={{ background: 'var(--s2)', border: '1px solid var(--border2)', borderRadius: 'var(--r)', padding: '14px 16px', marginBottom: 22 }}>
        <div style={{ fontSize: 'calc(10px * var(--font-scale))', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, color: 'var(--t3)', marginBottom: 10 }}>
          ...
          {items.map((item) => (
            <div key={item.product.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 'calc(12px * var(--font-scale))', color: 'var(--t2)' }}>{item.cutType.name} · ~{item.weightKg}kg</div>
              </div>
            </div>
          ))}
        </div>
        {items.length > 1 && (
          <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--border2)', fontSize: 'calc(13px * var(--font-scale))', fontWeight: 600, color: 'var(--gold)', textAlign: 'right' }}>
            Estimativa ~R$ {totalEstimate.toFixed(2).replace('.', ',')}
          </div>
        )}
      </div>

      <label style={{ fontSize: 'calc(11px * var(--font-scale))', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, color: 'var(--t3)', display: 'block', marginBottom: 7 }}>
        Número com DDD
      </label>
      ...
      <p style={{ fontSize: 'calc(12px * var(--font-scale))', color: 'var(--t3)', textAlign: 'center', marginTop: 16, lineHeight: 1.5 }}>
