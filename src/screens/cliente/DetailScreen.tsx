    ...
    <div className="scroll" style={{ flex: 1, paddingBottom: 28 }}>
      ...
      <div style={{ padding: '0 24px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16 }}>
          <span className="stars" style={{ fontSize: 14 }}>{'★'.repeat(Math.round(p.rating))}</span>
          <span style={{ fontSize: 13, fontWeight: 600 }}>{p.rating.toFixed(1)}</span>
          <span style={{ fontSize: 12, color: 'var(--t3)' }}>({p.reviews} avaliações)</span>
        </div>
        <div style={{ fontSize: 13.5, color: 'var(--t2)', lineHeight: 1.65, marginBottom: 18 }}>{p.description}</div>

        {/* Tipo de corte */}
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, color: 'var(--t3)', marginBottom: 10 }}>
            Tipo de Corte
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {p.cutTypes.map((ct) => (
              ...
              >
                <div style={{ fontSize: 13, fontWeight: 600, color: selectedCut.id === ct.id ? '#FF9090' : 'var(--t1)' }}>{ct.name}</div>
                <div style={{ fontSize: 10.5, color: 'var(--t3)', marginTop: 2 }}>{ct.desc}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 7, marginBottom: 22, flexWrap: 'wrap' }}>
          {p.tags.map((tag) => (
            <div key={tag} style={{ padding: '5px 12px', borderRadius: 20, background: 'var(--s2)', border: '1px solid var(--border2)', fontSize: 11.5, color: 'var(--t2)', fontWeight: 500 }}>
              {tag}
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 11, color: 'var(--t3)', marginBottom: 3 }}>Preço por kg</div>
            <div style={{ fontFamily: 'var(--font-serif)', fontSize: 36, fontWeight: 700, color: 'var(--accent)' }}>
              <sup style={{ fontSize: 18 }}>R$</sup>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, background: 'var(--s2)', border: '1px solid var(--border)', borderRadius: 14, padding: '8px 14px' }}>
            <div onClick={() => setWeight((w) => Math.max(0.5, +(w - 0.5).toFixed(1)))} style={{ width: 28, height: 28, borderRadius: 8, background: 'var(--s3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17, cursor: 'pointer', color: 'var(--t1)' }}>−</div>
            <div style={{ fontSize: 16, fontWeight: 700, minWidth: 52, textAlign: 'center' }}>
              {weight % 1 === 0 ? weight.toFixed(0) : weight.toFixed(1).replace('.', ',')} kg
            </div>
            <div onClick={() => setWeight((w) => +(w + 0.5).toFixed(1))} style={{ width: 28, height: 28, borderRadius: 8, background: 'var(--s3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17, cursor: 'pointer', color: 'var(--t1)' }}>+</div>
          </div>
        </div>
        ...
        <div style={{ background: 'var(--s2)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: '14px 16px', marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
            <span style={{ fontSize: 12, color: 'var(--t3)' }}>Estimativa ({weight.toFixed(1).replace('.', ',')} kg)</span>
            <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--accent)' }}>~{estimated}</span>
          </div>
          <div style={{ fontSize: 11.5, color: 'var(--t3)' }}>Peso exato pesado na hora · preço definitivo no caixa</div>
        </div>
